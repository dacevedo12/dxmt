...
// DXSO bytecode iterator — walks the body tokens (everything after
// the DxsoHeader version DWORD up to the END terminator) and yields
// one DxsoInstruction per step, with decoded operands for:
//   - the standard ALU shape (Mov, Mad, ...): dst, optional
//     predicate, sources
//   - the control-flow shape (If, Ifc, Rep, Loop, BreakC, BreakP):
//     sources only
//   - Dcl: semantic + dst register
//   - Def / DefI / DefB: dst register + typed literal payload
//     (Float32 / Int32 / Bool)
// Comment / End / Phase have no operands. Relative addressing
// (`c[a0+N]`-style indexing) is handled inline: when an operand
// token has bit 13 set, the iterator consumes the next DWORD as a
// DxsoRelativeReg per SM-version rules (SM3+ on dst, SM2+ on src).
//
// References: DXVK src/dxso/dxso_decoder.cpp
// (DxsoDecodeContext::decodeInstructionLength + decodeInstruction)
// and dxso_tables.cpp (DxsoGetDefaultOpcodeLength). wined3d
// shader_sm1.c carries the same default-length facts in
// shader_sm1_opcode_table[]. The instruction-count field is in
// token bits 24..27 for SM2.0+ and implicit per-opcode for SM1.x —
// Phase + Comment + End each have their own quirks, all faithfully
// reproduced below.
...
// per DXVK src/dxso/dxso_decoder.cpp:125-126; range is -8..+7.
...
// DXVK src/dxso/dxso_decoder.cpp:67-73.
...
// dxso_decoder.cpp:82-90. The naive `(type << 28)` encoding silently
...
// DxsoMaxOperandCount (src/dxso/dxso_decoder.h:12) — wide enough for
...
// mapping mirrors DXVK src/dxso/dxso_decoder.h:64 (DxsoOpcodeSpecificData):
...
struct DxsoInstruction {
  ...
  // Decoded operands. Filled for the "default" instruction shape
  // (dst + optional predicate + sources) and the control-flow shape
  // (sources only — If/Ifc/Rep/Loop/BreakC/BreakP). Dcl/Def/DefI/
  // DefB/Comment/End/Phase keep has_dst=false / src_count=0; the
  // iterator still steps past their body, but operand decoding for
  // those shapes (semantic tokens, constant literals) is a follow-up
  // commit.
  bool has_dst;
  DxsoDstRegister dst;
  bool has_predicate;
...
// Decode the per-opcode operand shape into out. body points at the
// first body DWORD (one past the opcode token); body_count is the
// total number of body DWORDs (including any relative-address extra
// DWORDs — DXVK's encoded length already accounts for them).
//
// major_version drives the relative-addressing rules:
//   - SM3.0+ allows rel-addr on destination registers
//   - SM2.0+ allows rel-addr on source registers
//   - rel-addr on the predicate slot is not supported (DXVK 240-243)
//
// Mirrors DXVK dxso_decoder.cpp:178-258 — the body's per-DWORD shape
// depends on the opcode group, not on the opcode-length encoding.
inline void
dxso_decode_operands(
  ...
  auto take_src = [&]() {
    ...
    if (s.has_relative && major_version >= 2 && cursor < body_count)
      s.relative = dxso_decode_relative(body[cursor++]);
    else
      s.has_relative = false;
  ...
  next(DxsoInstruction &out) {
    ...
    if (opcode == DxsoOpcode::Comment) {
      ...
      if (m_major >= 2) {
        ...
        // extra DWORD. DXVK dxso_decoder.cpp:44-54.
...
// Per-shader metadata gathered by walking the bytecode once at
// CreateShader time. Today this gives us validation (the iterator
// must reach End cleanly) and a starting point for AIR emission;
// later commits will extend it with input/output signature info,
// register usage masks, sampler layout, etc.
//
// uses_kill  — shader executes texkill at least once. AIR emit must
//              opt out of early fragment tests for kill-bearing PS
//              (texkill discards based on the source register sign,
//              and that decision can't be made before the FS runs).
// uses_derivatives — shader uses Tex / TexCoord / TexBem / DsX / DsY
//              / etc., i.e. an instruction that requires implicit
//              gradient computation. The list mirrors DXVK
//              src/dxso/dxso_analysis.cpp:24-48 verbatim — explicit-
//              LOD samples (TexLdd / TexLdl) are deliberately
//              excluded since they bring their own gradient.
...
// DXVK src/dxso/dxso_analysis.cpp:24-48; explicit-LOD samples
