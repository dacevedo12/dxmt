...
// DXSO header parser — reads the version token at byte_code[0] and
// classifies it. Used by Create{Vertex,Pixel}Shader to reject mal-
// formed bytecode at create time, before the shader handle is handed
// out and before any later AIR-emit pass tries to walk it. Reference:
// DXVK src/dxso/dxso_common.h (token layout) and the D3D9 shader
// bytecode reference.
//
// Header-only / no-LLVM: this file lives alongside the d3d9 module
// (not in airconv) so the mingw cross-built d3d9.dll can call it
// directly. The eventual DXSO→AIR translator will sit in airconv and
// re-use this header for version classification.
...
parse_dxso_header(const uint32_t *byte_code, size_t dword_count) {
  ...
  // Minor caps mirror DXVK d3d9_device.cpp:8150-8162: vs_1_x ≤ 1,
  ...
  // mirror that to avoid rejecting real-world FXC blobs (per
  // feedback_dxvk_first.md).
  if (major == 1) {
