...
compile_dxso(
  ...
  // Per-slot hardware-PCF flag, orthogonal to samp_kind. A shadow slot
  // keeps samp_kind == Texture2DDepth (the Metal kind is still depth2d);
  // this only flips the sample op to sample_compare and is set by the
  // host's _TEXTURE_2D_DEPTH_COMPARE sampler kind. Only the SM2+ Tex/
  // TexLdl/TexLdd arm consumes it — that's the texld/texldp path HW
  // shadow maps use. The legacy SM1.x texm3x*/texbem arms sample any
  // depth2d slot raw (their bound textures are color env/bump maps, never
  // shadow maps), which is consistent AIR (plain sample on depth2d simply
  // ignores the sampler's compareFunction).
  ...
  if (!is_vertex) {
    ...
    // bound to matching texture types. In practice, SM 1.4+/2.0+ PS can
    // declare `dcl_2d s0` while the engine binds a TextureCube to the
    // slot for an env-map effect. Trusting the dcl produced a 2D sampler
    // in MSL; Metal then saw a Cube view bound to a 2D sampler and failed
    // validation, with the visible effect being single-texel "flake"
    // patterns plus per-frame flicker (the precise symptom
    // MTL_SHADER_VALIDATION masks). wined3d's
  ...
  // variation on the unwritten lanes — e.g. a shader that writes
  // r0.rgb but leaves r0.a untouched produces undefined alpha that
  // flickers between frames; SRCALPHA / INVSRCALPHA blending then
  // turns that into a "flakes-plus-flicker" pattern. DXVK
  ...
  auto load_src = [&](const DxsoSrcRegister &src) -> Value * {
    ...
    case DxsoRegisterType::MiscType: {
      // PS-only built-ins. vPos (reg 0) = D3D9 pixel position: reuse the
      // [[position]] input (gl_FragCoord-equivalent), whose .xy are the
      // pixel coordinates the shader reads — DXVK maps MiscTypePosition the
      // same way. vFace (reg 1) = D3D9 face register, a float that is
      // positive when front-facing and negative when back; Metal's
      // [[front_facing]] is a bool over the same winding the raster state
      // was configured for, so map true -> +1, false -> -1. dxmt pins the
      // front-face winding to Clockwise (= D3D9 front) and does NO viewport
      // Y-flip (d3d9_device.cpp), so Metal [[front_facing]]==true is D3D9
      // front directly — don't "correct" this sign to a Vulkan/DXVK mental
      // model, where the negative-viewport Y-flip inverts winding and the
      // compensation lands on the same select for the opposite reason.
      if (is_vertex)
        return nullptr;
    ...
    // PRE-swizzle modifiers: Dz/Dw divide the RAW register by its z/w
    // lane before the swizzle is applied. DXVK splits this out into
    // emitSrcOperandPreSwizzleModifiers (dxso_compiler.cpp:1609-1626) and
    // runs it before emitRegisterSwizzle; wined3d likewise divides by the
    // raw component (glsl_shader.c texld/texcrd). Applying the divide
    // post-swizzle divides by the wrong lane whenever the source carries
    // a non-identity swizzle, throwing projected texcoords to ±inf —
    // stretched "ray" geometry. (The previous comment here claimed DXVK
    // applied Dz/Dw post-swizzle; that was incorrect — verified against
    // emitRegisterLoad's pre/swizzle/post ordering.)
  ...
  auto store_dst = [&](const DxsoDstRegister &dst, Value *value) {
    ...
    // FFP blend clamps it. DXVK emitDstStore (dxso_compiler.h:503-504)
  ...
  while (it.next(ins)) {
    ...
    case DxsoOpcode::DefB:
      break;
    case DxsoOpcode::Mov:
      // vs_1_x writes the VS address register with `mov a0.x, src` — there
      // is no `mova` opcode before SM2.0. store_dst drops writes to the
      // Addr register file for a vertex shader (it only handles PS texture
      // regs at that enum value), so without this a0 never updates and
      // every relative constant read `c[a0.x + N]` sees a0 = 0: skinned
      // meshes (matrix palette indexed by a0) collapse onto bone 0 —
      // T-pose with joints far from the root detached. Convert float→int
      // by FLOOR and store into a0, masked, exactly as wined3d does
      // (glsl_shader.c:4098-4111: `int(floor())` when shader_version.major
      // == 1 && dst is WINED3DSPR_ADDR; `mova`/SM2+ rounds). DXVK
      // (dxso_compiler.cpp:1851) floors only vs_1_0/1_1 and rounds
      // 1_2..1_4, but wined3d floors every vs_1_x and per the project's
      // reference priority wined3d wins. mov-to-a0 is illegal in SM2+
      // (FXC emits mova there), so the major==1 guard scopes it precisely.
    ...
    case DxsoOpcode::ExpP:
      // SM<2 partial-precision exp has a SPECIAL four-lane result that is
      // NOT a per-lane exp2 (DXVK dxso_compiler.cpp:2009-2032). With s the
      // scalar source (x lane of the swizzled/modified operand):
      //   .x = 2^floor(s)   .y = s - floor(s) (frac)   .z = 2^s   .w = 1
      // store_dst's writemask then selects which lanes land. Collapsing
      // all lanes to 2^s (the old behaviour) breaks any shader that reads
      // the .y (frac) lane — a skinning VS that extracts a bone index's
      // fractional part with `expp r.y, idx` to floor it will read a
      // wrong value from .y, driving the matrix-palette index out of
      // range and smearing bone-weighted vertices into rays. SM2+ ExpP
      // is plain exp2 (DXVK falls through to Exp).
    ...
    case DxsoOpcode::TexM3x2Tex: {
      ...
      // PS 1.x car-paint / environment-map shaders commonly use
      // TexM3x3Tex to look up tangent-space cube reflections; silently
