...
/* Per-stage texture kind for the PS sampler layout. Values mirror
   ...
   .r with .gba documented as "undefined", which is what produces the
   visible flake-and-flicker pattern on NFS:MW soft-particle smoke
   draws. depth2d returns a single float that we wire back to all four
...
/* PS-only specialisation: per-stage texture kind. SM 1.0..1.3 PS has
   ...
   dxso_compile.cpp. When the game binds a non-2D texture (e.g. NFS:MW's
   env-map cube for car reflections) to a shader compiled as 2D, Metal's
   shader validation flags the type mismatch and the GPU samples
   undefined data — visible flicker on the affected draws.
...
/* VS-only specialisation: D3DRS_POINTSIZE auto-injection. When this
   ...
   default-variant cache slot. DXVK src/dxso/dxso_compiler.cpp:3523-3538
...
/* PS-only specialisation: per-stage D3DTSS_BUMPENV* constants for
   ...
   DXVK src/dxso/dxso_compiler.cpp:1868-1905 + 2790-2803 + 2968-2987
