...
struct D3D9StateBlockChanges {
  ...
  // per-element divergence (see project_d3d9_stateblock_mask.md).
  void
...
public:
  ...
  // Phase 3.5c+ : public accessor to the underlying dxmt::CommandQueue.
  // The swapchain's Present routes its present-blit through a chunk on
  // this queue (chunk->emitcc + ctx.present + PresentBoundary). Returned
  // by reference because the queue owns its own encode/finish threads
  // and must outlive every chunk the device enqueues — the device owns
  // it via unique_ptr below.
...
private:
  ...
  // opt_cap (Phase 3.5a): when non-null, the validation-gate fields
  // (vs/ps/decl/rt0/ds) read from the capture instead of m_*. The
  // remaining device-state reads (render-states, samplers, textures,
  // streams, constants) still come from m_* — Phase 3.5b widens the
  // capture to cover those before flipping to async dispatch.
  // D3D9DrawCapture is defined further down in the class body; the
...
private:
  // Phase 3.4 — capture + batch infrastructure.
  //
  ...
  //
  // Phase ordering:
  //  - 3.4 (this commit): structs + queue + flush declarations only;
  //    DrawPrimitiveUP keeps calling drawCommonInScene directly.
  //  - 3.4-impl (next): BuildDrawCapture + sync FlushDrawBatch that
  //    drains pending_draws_ on the calling thread (validates capture
  //    correctness before async wiring).
  //  - 3.5: FlushDrawBatch becomes chunk->emitcc(lambda) — cap-lift.
public:
  ...
  struct D3D9DrawCapture {
    ...
    // texture path (project_dxmt_texture_buffer_backed_i386_trap)
    // has no Rc<dxmt::Texture> wrapper and must bind via raw handle.
    ...
    // decl, RTs/DS) lives on D9EncodingRefs (Phase 2B). Both are
    // populated by setter ops on the encode thread. Resolve reads
    // from the encode-side state directly.
    //
  ...
  struct BatchedDraw {
    ...
    // Phase 3 of the EmitOP port (~/.claude/plans/d3d9-emitop-port.md)
    // moved the resolve onto the encode thread; these fields are now
    // written from inside the chunk lambda just before
    // EmitDrawBatch_d9_chunk walks the batch. PSO build, IA layout,
    // viewFor, sampler/DSSO cache, RT/DS sRGB swap, vbuf-table upload
    // are all encode-thread work. The m_psoCache / m_samplerCache /
    ...
    // Per-stage resolved texture view + sampler handles. The view has
    // already been derived from cap.textures[stage] via viewFor (sRGB
    // swap honoured) on the calling thread.
    ...
    // ctx.access<Pixel>(rc, view, Read) to register a read dependency on
    // any prior encoder that wrote to this texture as an RT. Without it
    // the accumulated-batch path can let a sample-pass run in parallel
    // with the write-pass inside the same cmdbuf — visible as missing /
    // dim render-to-texture outputs (NFS:MW headlights, reflections,
    // post-process inputs). Null on buffer-backed paths (no wrapper);
    // those resources skip access tracking and may race.
  ...
  struct ChunkEmitState {
    ...
    // Shadow of the dxmt::Texture wrapper observed at each PS stage in
    // this encoder. ctx.access<Pixel>(rc, ...) registers the fence_wait
    // for cross-encoder dependency tracking — the registration is
    // idempotent within one encoder, so skip it when the same wrapper
    // is bound across consecutive draws. nullptr means "not yet seen
    // this encoder", which forces the first-draw access call.
...
private:
  ...
  // on the calling thread, per draw. Still fat as of Phase 3 of the
  // EmitOP port (~/.claude/plans/d3d9-emitop-port.md) — the slim-down
  // (carry only draw params + frozen rename-cursor offsets, read the
  // rest from encode-thread D9EncodingState) is deferred until the
  // ref-counted setters migrate into D9EncodingState (Phase 2B). The
  // override_* args (transient VB/IB for DrawPrimitiveUP / Ex) live
...
public:
  // Public for the swapchain's Present (Phase 3.6) which drains queued
  // draws onto a chunk before posting the present-blit chunk.
...
private:
  ...
  // Resolve a queued draw inside the chunk lambda. Phase 3 of the
  // EmitOP port (~/.claude/plans/d3d9-emitop-port.md) moved this from
  // the calling thread to the encode thread — the calling-thread cost
  // of per-draw IA / PSO / sampler / DSSO resolve was the dominant cap
  // on D3D9 hot-path throughput, per project_d3d9_loading_perf_rosetta.
  //
  ...
  struct ResolveCache {
    ...
    // allocation; on NFS:MW the cluster hit rate is ~80% but the table
    // contents are byte-identical across most of those clusters (no
  ...
  // Phase 3.8 fix-up: refresh m_cachedSignaled from m_completionEvent
  // and trim retired blocks out of m_constRing / m_uploadRing. Call
  // after every ++m_currentCmdSeq on the calling thread. The legacy
  // sync-cmdbuf finalize path used to do this implicitly; with that
  // path dead after 3.8, every chunk-emit site must refresh by hand
  // or the rings will burn fresh placed-buffer blocks per allocate
  // (one wine_unix_call per newBuffer, plus monotonic memory growth).
...
private:
  ...
  // Apple Silicon. Was 64 originally; bumped after a HUD capture
  // showed NFS:MW level transitions creating 100+ textures in one
  // frame burst — at 64 the pool evicted entries before the next
  // level's matching CreateTexture could hit them, so the streaming
  // pattern paid the cold-allocate cliff on every level boundary.
  ...
  // the rebuild cost from ~10 KB to ~5 KB in NFS:MW's typical heavy
  // race per-cluster pattern. Consecutive draws with no intervening
  ...
  // dxmt::CommandQueue — phase 1 of the async chunk pivot
  // (~/.claude/plans/d3d9-async-chunk-pivot.md). Constructed but not
  // yet routed to. The queue spins up its own encode + commit + event-
  // listener worker threads (idle until something posts a chunk).
  ...
  // Once Phase 3.5+ lands the chunk-emitcc draw lambda the wmtcmd
  // arena will stop touching these buffers from the calling thread,
  // and we'll be able to switch to the queue's shared allocator.
  ...
  // call. NFS:MW loading hammers UnlockRect at ~hundreds-per-frame;
  // each used to do one signaledValue thunk on the wow64 main thread,
  // contending with Rosetta's single-threaded exception server. Stale
  ...
  // VP6 path (9696 blit chunks in one HUD frame). Refresh at most
  // every kRingRefreshGap chunks; staleness delays ring-block recycle
  ...
  // (4 KB vs_const_F + 3.5 KB ps_const_F). NFS:MW typically sets
  // ~30 VS / ~20 PS regs against the 256/224 maxes — a ~16× reduction
  // in per-draw upload bandwidth. DXVK's `maxChangedConstF` is the
...
public:
  ...
  // calling thread's stack on i386 (project_d3d9_generatemipmaps_recursion).
  void generateMipmaps(WMT::Texture texture, bool drain_pending_draws = true);

  // Drain deferred blit work. Phase 3.8: the only remaining caller is
  // the AUTOGENMIPMAP-mipsDirty sweep — each dirty bound texture posts
  // its own generateMipmaps chunk (deduped within this call across
  // PS+VTF aliasing). Pending texture uploads emit chunks at
  // stageTextureUpload time and don't queue through here.
  ...
  // dominated NFS:MW loading (project_d3d9_loading_perf_rosetta).
