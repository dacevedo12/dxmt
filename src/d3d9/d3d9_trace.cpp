...
bool
D3D9CallTrace::trace_all() {
  static const bool en = std::getenv("DXMT_D9_TRACE_ALL") != nullptr;
  return en;
}

void
D3D9CallTrace::OnEveryCall(const char *name) {
  Logger::warn(std::string("[d9-hot] ") + name);
}
...
D3D9HotCounters::enabled() {
  // Gated on the log level (DXMT_LOG_LEVEL=debug or trace) rather than a
  // dedicated env var — the per-second telemetry is "debug" output. Cached
  // on first call: the log level is fixed at startup, and we touch this
  // from the hot path (every D9_HOT_* / D9_TRACE).
  static const bool en = DXMT_LOG_ENABLED(dxmt::LogLevel::Debug);
  return en;
}
...
D3D9HotCounters::tick() {
  ...
  struct {
    ...
    uint64_t n, total, mx, mn;
      ...
      {"cmdbufSubmit", 0, 0, 0, 0}, {"texCreate", 0, 0, 0, 0},   {"drawResolve", 0, 0, 0, 0},
  };
  ...
  drain_hist(c.drawResolve, hists[5].n, hists[5].total, hists[5].mx, hists[5].mn);
  char hline[640];
  ...
  // Live-object gauges — current alive count (not reset). A monotonic climb
  // across a capture localises a resource leak to a type.
  char gline[256];
  std::snprintf(
      gline, sizeof(gline), "live: textures=%lld surfaces=%lld vertexBuffers=%lld indexBuffers=%lld",
      (long long)c.liveTextures.load(std::memory_order_relaxed),
      (long long)c.liveSurfaces.load(std::memory_order_relaxed),
      (long long)c.liveVertexBuffers.load(std::memory_order_relaxed),
      (long long)c.liveIndexBuffers.load(std::memory_order_relaxed)
  );
  Logger::debug(gline);
}
