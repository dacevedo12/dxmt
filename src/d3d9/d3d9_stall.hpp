...
struct D9StallCounters {
  std::atomic<uint64_t> first_touch_ns{0};
  std::atomic<uint64_t> first_touch_bytes{0};
  ...
  void reset() {
    first_touch_ns.store(0, std::memory_order_relaxed);
    first_touch_bytes.store(0, std::memory_order_relaxed);
...
// Zero-fill that doubles as the first-touch of a freshly mapped placement: the
// page-fault cost a registered host buffer pays on first write is paid here, so
// timing this call measures exactly the per-allocation stall the streaming path
// incurs. Behaves as a plain memset when profiling is off.
inline void prefaultBacking(void *p, size_t len) {
  if (!d9StallThresholdMs()) {
    std::memset(p, 0, len);
    return;
  }
  ...
  auto ns = std::chrono::duration_cast<std::chrono::nanoseconds>(
                std::chrono::steady_clock::now() - t0)
                .count();
  g_d9stall.first_touch_ns.fetch_add(static_cast<uint64_t>(ns), std::memory_order_relaxed);
  g_d9stall.first_touch_bytes.fetch_add(len, std::memory_order_relaxed);
  g_d9stall.alloc_count.fetch_add(1, std::memory_order_relaxed);
}
