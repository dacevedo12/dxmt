...
// Drawable extent for the present blit = the host NSView's *live* client
// size in device pixels (GetClientRect * contentsScale). It is NOT
// MetalLayer_getProps().drawable_width: getProps reports the drawableSize
// we last pinned via setProps, which goes stale across a Reset. A
// fullscreen pass pins it to the display size, then a smaller windowed
// Reset reads the old display size straight back, and the present path
// bilinear-upscales the (correct, smaller) backbuffer to fill it — the
// blur seen switching fullscreen → smaller-windowed when the layer keeps
// a stale fullscreen drawable size.
//
// The window client rect tracks the actual host window, so a windowed
// game whose window matches its backbuffer presents 1:1 (present_blit_),
// while a legacy game whose backbuffer is smaller than its window still
// fits-to-window via present_scale_ — the reason d3d9 sizes off the
// window here rather than off the backbuffer like d3d11's ApplyLayerProps.
// Falls back to the backbuffer size when the HWND has no resolvable
// client area (headless / off-screen).
//
// The live-window poll is critical for windowed↔fullscreen transitions:
// the host NSView resizes asynchronously after Reset, so the layer's
// pinned drawableSize is stale until the OS completes the resize. Sizing
// off the live client rect lets the present path converge on the correct
// drawable size once the resize settles, avoiding present_scale_ upscaling
// a correctly-sized backbuffer into a stale (larger) drawable.
static void
layerDrawableExtent(
...
// failure so the caller keeps its prior value (d3d11_swapchain.cpp:191's
...
MTLD3D9SwapChain::Present(
  ...
  // Per-Present window-resize tracking (DXVK's model, d3d9_swapchain.cpp
  // GetWindowClientSize each PresentImage). The host NSView resizes
  // asynchronously AFTER the game changes its window, so a windowed↔
  // fullscreen toggle leaves the layer drawable at the pre-toggle size
  // for the Reset that requested it; sizing only at Reset reads a stale
  // window and present_scale_ ends up bilinear-upscaling a correct
  // backbuffer to fill the wrong-size layer. Re-apply the drawable extent
  // whenever the client rect moves so the layer snaps to the new window
  // once wine finishes the resize, and windowed mode lands back on a 1:1
  // present_blit_. Cheap: GetClientRect + two compares per Present;
  // changeLayerProperties (size-only, not PSO-invalidating) only fires
  // on an actual resize.
  if (m_hWindow) {
    uint32_t win_w = 0, win_h = 0;
  ...
  // shifting one position per Present.
  //
  ...
  m_presentationCount++;
  m_lastPresentReturn = std::chrono::steady_clock::now();
  // DXMT_LOG_LEVEL=trace: per-Present breakdown line — mirrors the
  // d3d11_swapchain.cpp:946-968 INFO log. The aggregate hotcounters
  // (DXMT_LOG_LEVEL=debug) hide single-frame spikes inside per-second
  // means; this surfaces each Present's encode / commit / boundary split
  // so an intermittent stall stops looking like steady-state.
  if (DXMT_LOG_ENABLED(dxmt::LogLevel::Trace)) {
