...
public:
  ...
  // Writable mirror base: UpdateTexture mirrors copied bytes into a DYNAMIC
  // destination's sysmem copy so a later LockBox (which serves the mirror) sees
  // them. Null when this pool has no mirror.
  uint8_t *
  mirrorWriteBase() {
    return m_mirror.empty() ? nullptr : m_mirror.data();
  }
