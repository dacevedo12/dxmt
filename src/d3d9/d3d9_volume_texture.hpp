...
// IDirect3DVolumeTexture9 — 3D texture with N mip levels, each a 3D
// (Width × Height × Depth) sub-resource. Used by D3D9 games for color-
// grading LUTs (most common modern use), volume rendering, and a
// handful of fluid-sim look-up tables.
//
// Shape mirrors MTLD3D9Texture but with WMTTextureType3D + a per-level
// MTLD3D9Volume sub-resource. CPU mirror is a plain std::vector<uint8_t>
// like MTLD3D9CubeTexture (no Metal-buffer wrapper) — UpdateTexture
// routes through stageTextureUpload for each level. Compressed
// formats are never legal on volume textures (DXT* are 2D-only in
// D3D9), so the mirror math stays a pure (rowPitch × Height × Depth)
// linear layout.
//
// Reference: wined3d dlls/d3d9/volume.c + texture.c
// d3d9_texture_3d_*. DXVK src/d3d9/d3d9_volume.h.
