...
MTLD3D9Surface::LockRect(D3DLOCKED_RECT *pLockedRect, const RECT *pRect, DWORD Flags) {
  ...
  if (m_locked)
    return D3DERR_INVALIDCALL;
  // DXVK d3d9_device.cpp:4460+ LockImage validation gates.
...
MTLD3D9Surface::UnlockRect() {
  ...
  // wined3d surface.c:278-286 softens the double-Unlock gate when the
  ...
  // DEFAULT-pool off-screen plain surfaces — see CreateOffscreenPlainSurface
  // — and DYNAMIC DEFAULT-pool textures — see d3d9_texture.cpp ensureMirror)
  // need an explicit upload to push the sysmem mirror into the Metal
  // texture. wined3d texture.c
  // d3d9_surface_unmap + wined3d_texture_upload_data is the same shape:
  // copy the mirror row-by-row into the GPU resource. SYSTEMMEM/SCRATCH
  // skip the push — those pools are buffer-backed (m_buffer != null,
  // excluded below) and the sysmem mirror IS the master; UpdateTexture
  // is the path from there to a DEFAULT mirror.
  //
  ...
  // GPU reads of the same texture on Apple Silicon Shared storage — a
  // software-video upload path that issues hundreds of LockRect /
  // UnlockRect per frame capped at ~1 fps with replaceRegion.
  if (m_buffer == nullptr && m_cpu_ptr != nullptr && m_texture != nullptr &&
    ...
    // ring's renaming step. A direct mirror-buffer blit (skipping the
    // memcpy) is faster but has a correctness hazard: the chunk lambda
    // holds the mirror handle and the GPU reads its bytes at execution
    // time, not at lambda-post time. On Apple Silicon UMA the mirror is
    // the same physical memory the next LockRect's pBits points at —
    // so a Lock(0)/Unlock cycle that races a still-pending upload chunk
    // overwrites bytes the GPU is about to read. Memcpy-into-ring
    // snapshots the bytes at Unlock time so the mirror is safe to
    // overwrite on the next Lock. A per-surface rename ring on the
    // mirror would recover the perf without the race; that's a follow-on.
