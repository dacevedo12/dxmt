...
// Per-axis dirty mask for the calling-thread → encode-side POD COW
// snapshot. Each POD setter ORs in its category bit on a state-changing
// write (the no-op gates short-circuit before the OR, matching the spec
// — DXVK d3d9_state.cpp's stage-1-bit-per-category model). QueueBatchedDraw
// then rebuilds the cached D9EncodingState by copying ONLY the dirty
// axes from the calling-thread shadow on top of the prior snapshot,
// instead of a 10 KB full memcpy. A typical draw-heavy frame only
// dirties VS_CONST_F | PS_CONST_F | SAMPLER_STATES (~5 KB) vs the
// 10.5 KB full payload.
...
// Per-draw POD snapshot of D3D9 device state. Stored on BatchedDraw
// via std::shared_ptr<D9EncodingState> (COW'd per state-change
// cluster). Resolve/Emit on the encode thread dereference the
// snapshot to derive all per-draw POD inputs frozen at queue time.
//
// Compact — only the fields Resolve / EmitCommonRenderSetup_d9 /
// EmitDrawCommand_d9 actually read today. transforms[266] (17 KB)
// and texture_stage_states[8][33] (1 KB) used to live here for the
// FFP shader generator's eventual benefit, but no draw-time reader
// exists yet, so they're parked on the calling-thread shadow and
// snapshotted lazily when FFP lands.
//
// Total size: ~10 KB (down from 27 KB pre-compaction). A draw-heavy
// scene sees ~200 unique POD clusters per frame; the compaction
// cuts per-frame snapshot bandwidth from ~5 MB to ~2 MB.
