...
// IDirect3DCubeTexture9 owns one Metal TextureCube allocation (6
// faces × N mip levels) and pre-creates 6×N MTLD3D9Surface views,
// one per face/level. The face surfaces share the parent's
// WMT::Texture handle; each surface stores its mipLevel + arraySlice
// so render-pass attachments (cube RTs) and sampler bindings select
// the right plane.
//
// Common use: small reflection probes in D3DPOOL_MANAGED, sampled
// via texld with .xyz coords. Apps gate ENV-probe rendering on
// CreateCubeTexture succeeding, so an E_NOTIMPL return blocks startup.
//
// Lockability mirrors MTLD3D9Texture: non-DEFAULT pools allocate a
// per-face/per-level sysmem mirror, hand back a CPU pointer from
// LockRect, and on MANAGED push the data to GPU via replaceRegion in
// UnlockRect using the surface's m_array_slice (face) + m_mip_level
// fields.
//
// Reference: wined3d dlls/d3d9/texture.c d3d9_texture_cube_*. MGL has
// no cube-specific lowering (Metal exposes texturecube as a top-level
// type in MSL, no per-face binding).
...
public:
  ...
  // See MTLD3D9Texture::dxmtTexture — Phase 3.5 chunk lambdas capture
  // this Rc<> to keep the cube alive across the calling-thread →
  // encode-thread boundary.
...
private:
  ...
  // Per-cube-texture view cache. Cube views with sRGB or swizzle
  // aliasing keep the WMTTextureTypeCube target so all 6 faces stay
  // addressable through the same view.
