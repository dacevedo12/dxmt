...
MTLD3D9Device::dummyFragmentTexture(WMTTextureType type) {
  // Zero-filled 1×1 placeholder bound to a fragment sampler slot the PS
  // samples with no app texture bound. The TYPE must match the sampler
  // kind the PS variant was compiled with: binding a 2D texture to a 3D
  // or cube sampler is a Metal type mismatch that samples undefined
  // (black) and trips validation. wined3d binds a per-type dummy for the
  // same reason (dlls/wined3d/device.c create_dummy_textures) and fills
  // it with 0, so an unbound sample reads transparent black; we mirror
  // both the per-type binding and the zero fill. Cached per type
  // (0 = 2D, 1 = 3D, 2 = Cube).
  uint32_t idx;
  switch (type) {
  ...
  // Shared (not Private) so the zero fill below is CPU-writable.
  info.options = WMTResourceStorageModeShared;
...
MTLD3D9Device::CreateTexture(
  ...
  *ppTexture = nullptr;
  ...
  // restricts SYSTEMMEM share to single-level (texture.c).
  if (pSharedHandle) {
    if (!m_isEx)
      return E_NOTIMPL;
    if (Pool == D3DPOOL_SYSTEMMEM) {
