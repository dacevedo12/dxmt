...
MTLD3D9Device::MTLD3D9Device(
  ...
  // and binds it to the device. Without it, scene draws land in
  // render passes with no depth attachment and produce a black screen.
  //
...
// only blocks whose seq has retired.
//
...
EmitTextureUploadChunk_d9(
  ...
  // RETAIN the destination texture for the deferred upload. The chunk lambda runs on the
  // encode thread LATER; the calling thread can drop dst's owner before then — a device
  // Reset tearing resources down, or the app releasing the texture. A raw handle would then
  // dangle and the blit would touch a freed Metal texture. NSZombie caught exactly this:
  // "-[MTLTexture storageMode]: message sent to deallocated instance" during a vsync-toggle
  // Reset (the "resource ownership graph outlives the GPU read" assumption this code used to
  // rely on is false across a Reset). Matches the other blit paths (StretchRect/ColorFill),
  // which already capture a *_tex_retain. The source buffer is an m_uploadRing block
  // (recycled by signaledValue) or a surface-owned MANAGED mirror, so its lifetime is
  // already covered by the ring / surface refcount.
...
MTLD3D9Device::Reset(D3DPRESENT_PARAMETERS *pPresentationParameters) {
  ...
  // use. Items not yet implemented vs a faithful wined3d / DXVK Reset,
  // tracked separately:
  ...
  //    pushes null refs so encode-thread D9EncodingRefs stay coherent.
...
MTLD3D9Device::CreateTexture(
  ...
  *ppTexture = nullptr;
  ...
  // Two ways the RT bit gets added:
  //  - D3DUSAGE_DEPTHSTENCIL: the Metal DS attachment is also a render
  //    target on Apple GPUs (shader-read stays for shadow-map sampling).
  //  - DEFAULT-pool colour textures: RT-capability promotion. DXVK does
  //    this unconditionally for every Vulkan colour image; we want the
  //    same so ColorFill / StretchRect-render-pass-blit work on plain
  //    CreateTexture(DEFAULT) sub-levels without a second descriptor
  //    path. The promotion subsumes explicit D3DUSAGE_RENDERTARGET —
  //    RT usage is gated to DEFAULT above, so any RT texture is also a
  //    DEFAULT-pool colour texture and the promotion covers it.
  // Skipped for compressed formats — BC textures can't be render-
  // targets on Apple Silicon; adding the bit makes newTexture reject
  // and CreateTexture report OUTOFVIDEOMEMORY for an asset that
  // should have loaded as a sampler-only resource.
  ...
  // buffer-backed fast path — UMA-correct MANAGED single-buffer.
  ...
  // per-Unlock memcpy disappears — the GPU samples directly from the
  // host-mapped buffer pages on UMA.
  //
  ...
  // Unified construction path — every texture is rooted in a single
  // Rc<dxmt::Texture> so chunk lambdas keep it alive across the
  // calling-thread → encode-thread → GPU-completion boundary via
  // ctx.access + ref_tracker. Two flavours:
  //   - Buffer-backed: caller manages the host page so
  //     LockRect can hand the i386 process the 32-bit-addressable
  //     pBits the game expects. We pull from a device-level reuse
  //     pool (or wsi::aligned_malloc fresh), wrap into a Metal
  //     buffer, then wrapBuffer the (buffer, mapped) pair into a
  //     dxmt::Texture allocation that owns both for its lifetime.
  //     dxmt::Texture::allocate's __i386__ branch would also work but
  //     would lose the pool / pre-fault optimisations the caller-
  //     supplied path keeps.
  //   - Regular: dxmt::Texture(info, device) + Texture::allocate, with
  //     the pool→TextureAllocationFlag mapping below (DEFAULT →
  //     CpuInvisible / Private, others → Shared / default cache).
  // The same MTLD3D9Texture ctor (Rc<dxmt::Texture> + bufferPitch)
  // serves both — bufferPitch is non-zero only for the buffer-backed
  // flavour and signals buildLevelsAndMirror to alias level 0 onto
  // the backing instead of allocating a sysmem mirror.
  ...
  if (buffer_backed_eligible) {
    ...
    backingPitch = static_cast<uint32_t>((row_bytes + alignment - 1) & ~(alignment - 1));
    ...
    // CreateVertexBuffer. Streaming workloads create textures of recurring
    // sizes; on a pool hit we skip the newBuffer XPC AND the page-fault
    // cliff that the pre-fault memset would otherwise pay during gameplay.
    // The pool is fed by VB/IB and
    // texture-mirror dtors today; buffer-backed-texture donation is
    // deferred until TextureAllocation gains a post-completion hook
    // (donating from ~MTLD3D9Texture is unsafe — in-flight chunks may
    // still retain the allocation via the ref_tracker).
...
MTLD3D9Device::CreateVolumeTexture(
  ...
  *ppVolumeTexture = nullptr;
  ...
  // texture.c:1265-1271 (d3d9_texture_init) rejects with INVALIDCALL on
  // WINED3D_RTYPE_TEXTURE_3D; DXVK d3d9_common_texture.cpp:265 mirrors
...
MTLD3D9Device::CreateVertexBuffer(
  ...
  *ppVertexBuffer = nullptr;
  ...
  // the 4 GB line that 32-bit Windows games can't reach. Pre-allocate
  // the backing in process address space and hand it to Metal via
  ...
  // is physically impossible under the new
...
MTLD3D9Device::UpdateSurface(
  ...
  // the consumer).
  // AUTOGENMIPMAP regen — DXVK d3d9_device.cpp:1075-1076 flags the
  // dest texture's auto-gen mipmap state via MarkTextureMipsDirty
  // after a successful UpdateSurface; same shape as StretchRect /
  // Clear (see MTLD3D9Surface::flagContainerAutoGenDirty). The
  // SetNeedsReadback half of DXVK :1073 is deferred — dxmt has no
  // mapped-image cache that depends on the readback flag.
...
//
// All three resource types (2D, Cube, Volume) ship implementations
// below; only D3DRTYPE_SURFACE and similar non-texture types fall
// through to D3DERR_NOTAVAILABLE. Each branch consumes its mirror's
// dirty-region bookkeeping to scope the upload to the touched
// sub-rect rather than the full level extent.
HRESULT STDMETHODCALLTYPE
MTLD3D9Device::UpdateTexture(IDirect3DBaseTexture9 *pSourceTexture, IDirect3DBaseTexture9 *pDestinationTexture) {
  ...
  // autoreleased handles. Wine's main thread has no outer NSAutoreleasePool.
  auto pool = WMT::MakeAutoreleasePool();
  ...
  // dst==DEFAULT). Liberal-accept is a deliberate fork: some apps pass
  // MANAGED on at least one side and depend on success — tightening to
  // spec breaks them. The
  ...
  if (src_type == D3DRTYPE_TEXTURE) {
    ...
    src->clearDirty();
    // wined3d device.c:1927 flags the dst's auto-gen mipmap state after
  ...
  // rather than the buffer-direct path.
  {
    ...
    // device.c:1927). Flag the dst's lazy mipmap-dirty bit; no-op
...
MTLD3D9Device::GetRenderTargetData(IDirect3DSurface9 *pRenderTarget, IDirect3DSurface9 *pDestSurface) {
  ...
  // Drain queued draws onto a chunk first so the read sees their writes.
  // Then post the blit as its own chunk lambda + wait on the chunk's
  // completion before returning so the caller's next LockRect sees fresh
  // bytes.
  if (!m_pendingOps.empty())
    FlushDrawBatch();
...
MTLD3D9Device::GetFrontBufferData(UINT iSwapChain, IDirect3DSurface9 *pDestSurface) {
  ...
  // Same shape as GetRenderTargetData — drain pending draws onto a chunk,
  // post the blit on its own chunk, wait for it.
  if (!m_pendingOps.empty())
    FlushDrawBatch();
...
MTLD3D9Device::StretchRect(
  ...
  // device.c:2038-2058 rejects partial src_rect / dst_rect, DXVK
  // d3d9_device.cpp:1385-1388 rejects when srcCopyExtent != srcExtent.
  ...
  // wined3d device.c:2066 — flag the dest texture's auto-gen mipmap
...
MTLD3D9Device::ColorFill(IDirect3DSurface9 *pSurface, const RECT *pRect, D3DCOLOR Color) {
  ...
  ++m_currentCmdSeq;
  refreshSignaledAndTrimRings();
  // AUTOGENMIPMAP regen — DXVK d3d9_device.cpp:1640 calls
...
MTLD3D9Device::CreateOffscreenPlainSurface(
  ...
  *ppSurface = nullptr;
  ...
  // is 32-bit-unsafe: dxmt::Texture's buffer-backed allocate() uses
  // Metal-allocated memory that may land above the 4 GB line.
  Rc<dxmt::Texture> dxmt_texture;
  ...
  if (Pool == D3DPOOL_DEFAULT) {
    ...
    dxmt_texture->rename(std::move(allocation));
    // MSDN: off-screen plain surfaces are ALWAYS lockable, even in
    // D3DPOOL_DEFAULT. The Private texture above can't carry a CPU
    // pointer, and we can't make it buffer-backed without losing the
    // RenderTarget usage a DEFAULT off-screen surface needs to be a
    // render-pass StretchRect destination (Apple Silicon disallows the
    // RenderTarget usage on a linear/buffer-backed texture). So give it a
    // host-side mirror the same shape MANAGED textures use: LockRect
    // hands back the mirror, UnlockRect uploads the dirty rect into the
    // Private texture. Games create a DEFAULT off-screen surface, lock +
    // fill it, then StretchRect it to the back buffer (e.g. a video-
    // decode path that Com_Error(ERR_FATAL)s if the lock fails). Read
    // locks return the mirror (zeroed until first written) — a DEFAULT
    // off-screen surface that is rendered to and then read back would see
    // stale mirror bytes, but the lock-fill-then-blit pattern that real
    // games use is correct.
    ...
    // Pre-allocate the backing in process address space and hand it
    // to Metal via
...
MTLD3D9Device::Clear(DWORD Count, const D3DRECT *pRects, DWORD Flags, D3DCOLOR Color, float Z, DWORD Stencil) {
  ...
  if (Count == 0 && pRects != nullptr)
    return D3D_OK;
  // Non-zero Count with a NULL rect array: both references normalise the
  // count to zero and clear the full target rather than reject — wined3d
  // (device.c d3d9_device_Clear: "count with NULL rects" → rect_count = 0)
  // and DXVK (d3d9_device.cpp Clear: if (!pRects) Count = 0). Games that
  // pass a stale count with no rects expect the full-target clear; the
  // prior D3DERR_INVALIDCALL diverged from both and rejected a legal call
  // (a dropped frame-clear → visual garbage). dxmt widens every clear to
  // the full target, so normalising Count keeps the per-rect emit path
  // (when it lands) from dereferencing a null pRects.
  if (Count != 0 && pRects == nullptr)
    Count = 0;
  ...
  // wined3d device.c:2378 calls d3d9_rts_flag_auto_gen_mipmap after a
...
MTLD3D9Device::ValidateDevice(DWORD *pNumPasses) {
  ...
  // so we always claim single-pass. Neither reference rejects a NULL
  // out-pointer: DXVK (d3d9_device.cpp ValidateDevice) writes *pNumPasses
  // only when non-null and returns D3D_OK, and wined3d's d3d9 layer
  // forwards the pointer through without a gate. The prior INVALIDCALL
  // gate cited a DXVK behaviour that does not exist; match the
  // references and treat NULL as "result not wanted".
  if (pNumPasses)
    *pNumPasses = 1;
  return D3D_OK;
}
...
// EmitDrawCommand_d9. Every draw rides one shared chunk; per-(RT,DS)
// encoder batching avoids tile-store/load on AGX
...
// drawCommonInScene's validation gate (vs/ps/decl/rt0 null-check) reads
// from the capture when the caller provides one. UP paths build a capture
// at queue time so FlushDrawBatch's per-pending-draw drawCommonInScene
// call is validated against the same shader/RT bindings the calling
// thread observed at draw time, even if FlushDrawBatch runs after
// Set*Shader / SetRenderTarget mutated the device state.
MTLD3D9Device::D3D9DrawCapture
MTLD3D9Device::BuildDrawCapture() {
  // POD state (render_states, sampler_states, stream_freq, viewport,
  // scissor, clip_planes, VS/PS constants) is read by Resolve from
  // D9EncodingState on the encode thread; the setter-flush invariant in
  // each POD setter ensures all draws in a batch share one POD snapshot.
  // Ref-counted state (vs/ps/decl/textures/VBs/IB/RTs/DS) travels via
  // setter ops into m_d9EncRefs. BuildDrawCapture's only job is freezing
  // per-draw rename cursors — gpu_address() / currentOffset() advance on
  // Lock(DISCARD), so they must be snapshotted at queue time, not at
  // Resolve time. Buffer handles are stable across rename moves but kept
  // here so every per-stream value comes from a single Build-time snapshot.
  D3D9DrawCapture cap;
...
MTLD3D9Device::QueueBatchedDraw(BatchedDraw &&draw) {
  ...
  // calling-thread shadow. A typical heavy-draw cluster touches ~3 axes
  // (VS_CONST_F | PS_CONST_F | SAMPLER_STATES), so this cuts the rebuild
  // from a ~20 KB write set (zero + full memcpy) to ~13 KB (one POD
  // copy + a few-KB overwrite).
  //
...
EmitCommonRenderSetup_d9(
  ...
  for (uint32_t stage = 0; stage < 16; ++stage) {
    ...
    obj_handle_t mt;
    if (rc_ptr) {
      // App texture: resolve + fence-track the per-bind view.
      // ctx.access(viewId) returns the TextureView AND retains the
      // allocation that OWNS the view object (cached_view_), so the
      // view survives a Reset dropping the MTLD3D9Texture wrapper mid-
      // flight — the storageMode UAF the unified-allocation refactor closes.
      // The registration
      // is idempotent within one encoder, so skip the re-access when the
      // same (wrapper, view) is already bound this encoder; keying on the
      // view too means a SetLOD / sRGB-toggle / swizzle change re-accesses
      // and binds the correct new view handle.
...
EmitBlitOp_d9(ArgumentEncodingContext &ctx, MTLD3D9Device::PendingBlitOp &op) {
  ...
  // executing the blit BEFORE either render writes A and leaving the
  // destination empty for the draw passes.
  // d3d11_context_impl.cpp:4137-4147 (CopySubresource
...
MTLD3D9Device::ResolveBatchedDrawForChunk(
  ...
  // key / topology / ib_fmt. Hit ratio mirrors the const-cache at ~80%,
  // but the savings per hit are much bigger:
  ...
  if (cluster_hit) {
    ...
    // flicker (cube-map env-map reflections). dcl-bearing shaders
    ...
    for (uint32_t stage = 0; stage < 16; ++stage) {
      ...
      case D3DRTYPE_TEXTURE:
        ...
        // undefined" — the undefined channels produce visible smoke
        // flake/flicker artefacts. Force the depth2d<float>
    ...
    for (uint32_t stage = 0; stage < 16; ++stage) {
      ...
      if (!tex) {
        // No app texture at this stage. A PS that samples this slot
        // (SM<1.4 defaults every sampler to Texture2D in dxso_compile)
        // would otherwise leave the fragment texture/sampler index
        // unbound -> Metal "missing Sampler binding at index N" / the GPU
        // kIOGPUCommandBufferCallbackErrorInvalidResource. This triggers
        // after a Reset that clears all textures when the first
        // post-Reset sampling draw lands before the app re-binds its
        // texture. Bind a 1x1 placeholder texture + the stage's sampler
        // (which exists regardless of a bound texture) so the encoder is
        // complete. resolved_frag_texture_dxmt stays null: the placeholder
        // is immutable, so it needs no fence-tracked access registration.
        ...
        bd.resolved_frag_textures[stage] = dummyFragmentTexture2D();
        continue;
      }
      // Build the per-bind sample view through the dxmt::Texture view
      // system (was the per-wrapper D3D9ViewCache). The view object now
      // lives on the TextureAllocation, which the chunk ref_tracker
      // retains to GPU completion via the access(viewId) call below — so
      // it survives the wrapper being dropped on a Reset that lands mid-
      // flight. Chain the
      // derivations off fullView; each checkViewUse* is a no-op when its
      // axis already matches (identity swizzle / no sRGB / lod 0), so the
      // common case stays at fullView and binds the base texture.
  ...
  // Cache the prior draw's slot_mask + per-slot (base_addr, stride,
  // length) on ResolveCache. On match — typical for clusters of draws
  // that share a VB binding with no SetStreamSource between them —
  // reuse the cached (buffer, offset) and skip the m_constRing.allocate
  // + per-slot store. With ~80% cluster hit rate this avoids ~800 ring
  // allocates/frame. Compute first into stack-local arrays, then memcmp;
  // on miss copy stack → ring + update cache.
  struct VbufEntry {
    uint64_t base_addr;
    uint32_t stride;
    uint32_t length;
  ...
  // or alias the offset. With ~80% POD-cluster hit rate, ~80% of draws
  // skip an 8 KB memcpy + mutex pair.
...
MTLD3D9Device::FlushDrawBatch() {
  ...
  // against the i386 process heap). At ~1000 draws/frame and
  // sizeof(BatchedDraw) ~1.2-1.5 KB that amounts to ~5-10 MB churn/frame.
  ...
  chunk->emitcc([this, ops = std::move(m_pendingOps), draws = std::move(m_pendingDraws),
    ...
    for (auto &ref : ops) {
      ...
      if (ref.kind == PendingOpRef::Draw) {
        ...
        // Per-draw ENCODE-thread cost (T0.3 drawResolve bucket): the whole
        // Draw block — ResolveBatchedDrawForChunk + PSO wait + emit. Pairs
        // with the calling-thread drawEncode histogram so a capture accounts
        // for 100% of per-draw work; the PSO-wait slice is isolated by
        // psoWaitMicros. RAII so every `continue` path below still records.
        D9_HOT_SCOPE(drawResolve);
...
MTLD3D9Device::refreshSignaledAndTrimRings() {
  ...
  // blit-heavy cutscene paths can post thousands of chunk-emits in one
  // frame; refreshing per call multiplies the unix-call rate
...
// itself lives in build_fvf_decl_elements (d3d9_fvf.cpp).
HRESULT STDMETHODCALLTYPE
