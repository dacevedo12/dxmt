...
EmitTextureUploadChunk_d9(
    MTLD3D9Device *self, dxmt::CommandQueue &queue, WMT::Texture dst, const Rc<dxmt::Texture> &dst_alloc,
    obj_handle_t src_buffer_handle, uint64_t src_offset, uint32_t src_pitch, uint32_t src_bytes_per_image,
    uint32_t mip_level, uint32_t slice, WMTOrigin origin, WMTSize size
) {
  ...
  chunk->emitcc([dst_retain = std::move(dst_retain), dst_alloc, dst_handle, src_buffer_handle, src_offset, src_pitch,
                 src_bytes_per_image, mip_level, slice, origin, size](ArgumentEncodingContext &ctx) mutable {
    ctx.startBlitPass();
    // Register the destination write so the encoder scheduler keeps this
    // upload ahead of any later pass that samples the texture. Without it the
    // blit-merge step floats the upload past a same-chunk render pass that
    // reads the texture (stale-read; a VS texture-fetch draw exposed it). Same
    // dependency the StretchRect blit op registers via access<Compute>.
    if (dst_alloc.ptr())
      ctx.access<PipelineStage::Compute>(dst_alloc, mip_level, slice, ResourceAccess::Write);
...
MTLD3D9Device::stageTextureUpload(
  ...
  EmitTextureUploadChunk_d9(
      this, *m_dxmtQueue, dst, dst_alloc, block.buffer.handle, offset, src_pitch, bytes_per_image, mip_level, slice,
      origin, size
  );
  ++m_currentCmdSeq;
  refreshSignaledAndTrimRings();
...
MTLD3D9Device::GetAvailableTextureMem() {
  ...
  // half of recommendedMaxWorkingSetSize.
  uint64_t bytes = m_metalDevice.recommendedMaxWorkingSetSize();
  if (m_metalDevice.hasUnifiedMemory())
    bytes /= 2;
  // Cap below 4 GB so the result fits UINT and, more importantly, so the
  // running allocation counter below stays visible: a multi-GB UMA working set
  // would otherwise saturate the UINT and never appear to change as resources
  // are created. 0xfff00000 keeps MB granularity (matches DXVK's mask).
  ...
  return static_cast<UINT>(bytes) & 0xfff00000u;
}
HRESULT STDMETHODCALLTYPE
...
MTLD3D9Device::CreateTexture(
  ...
  *ppTexture = nullptr;
  ...
  // D3DUSAGE_DYNAMIC is incompatible with MANAGED: a managed resource keeps a
  // sysmem master with its own upload model, so the dynamic hint cannot apply.
  // DXVK rejects MANAGED+DYNAMIC in NormalizeTextureProperties; wined3d rejects
  // it at resource init. SCRATCH+DYNAMIC stays valid here: only volumes reject
  // it (see CreateVolumeTexture), 2D and cube do not.
  if ((Usage & D3DUSAGE_DYNAMIC) && Pool == D3DPOOL_MANAGED)
    return D3DERR_INVALIDCALL;
  // RT/DS bind is incompatible with DYNAMIC: a render target or depth surface
  // is GPU-written, which the dynamic upload model cannot back (wined3d
  // resource.c rejects the RT/DS-with-DYNAMIC pair at resource init).
  if ((Usage & D3DUSAGE_DYNAMIC) && (Usage & (D3DUSAGE_RENDERTARGET | D3DUSAGE_DEPTHSTENCIL)))
    return D3DERR_INVALIDCALL;
...
MTLD3D9Device::CreateVolumeTexture(
  ...
  *ppVolumeTexture = nullptr;
  ...
  // wined3d texture.c; D3DPOOL_MANAGED is invalid on d3d9ex. The 2D and
  // cube create paths reject it already; this is the missing sibling.
  if (Pool == D3DPOOL_MANAGED && m_isEx)
    return D3DERR_INVALIDCALL;
  ...
  if (Usage & D3DUSAGE_AUTOGENMIPMAP)
    return D3DERR_INVALIDCALL;
  // Volumes reject DYNAMIC with both MANAGED and SCRATCH. MANAGED keeps a sysmem
  // master with its own upload model; SCRATCH+DYNAMIC is rejected for volumes
  // specifically (DXVK scopes that reject to volume textures, and the wine d3d9
  // test asserts it), unlike the 2D and cube paths which permit SCRATCH+DYNAMIC.
  if ((Usage & D3DUSAGE_DYNAMIC) && (Pool == D3DPOOL_MANAGED || Pool == D3DPOOL_SCRATCH))
    return D3DERR_INVALIDCALL;
...
MTLD3D9Device::CreateCubeTexture(
  ...
  *ppCubeTexture = nullptr;
  ...
  if ((Usage & (D3DUSAGE_RENDERTARGET | D3DUSAGE_DEPTHSTENCIL)) && Pool != D3DPOOL_DEFAULT)
    return D3DERR_INVALIDCALL;
  // DYNAMIC is incompatible with MANAGED and with RT/DS bind; see CreateTexture.
  if ((Usage & D3DUSAGE_DYNAMIC) && Pool == D3DPOOL_MANAGED)
    return D3DERR_INVALIDCALL;
  if ((Usage & D3DUSAGE_DYNAMIC) && (Usage & (D3DUSAGE_RENDERTARGET | D3DUSAGE_DEPTHSTENCIL)))
    return D3DERR_INVALIDCALL;
...
MTLD3D9Device::CreateVertexBuffer(
  ...
  *ppVertexBuffer = nullptr;
  ...
  // contract, unlike the texture paths), ahead of everything below.
  // Cross-process buffer sharing itself is unimplemented.
  if (pSharedHandle) {
    if (!m_isEx)
      return E_NOTIMPL;
    if (Pool != D3DPOOL_DEFAULT)
...
MTLD3D9Device::CreateIndexBuffer(
  ...
  *ppIndexBuffer = nullptr;
  ...
  // format check below. Cross-process buffer sharing itself is unimplemented.
  if (pSharedHandle) {
    if (!m_isEx)
      return E_NOTIMPL;
    if (Pool != D3DPOOL_DEFAULT)
...
MTLD3D9Device::UpdateSurface(
  ...
  if (void *src_host = src->cpuPtr()) {
    ...
    stageTextureUpload(
        dst->metalTexture(), dst->dxmtTexture(), dst->mipLevel(), /*slice=*/0, WMTOrigin{dst_x0, dst_y0, 0},
...
MTLD3D9Device::UpdateTexture(IDirect3DBaseTexture9 *pSourceTexture, IDirect3DBaseTexture9 *pDestinationTexture) {
  ...
  if (src_type == D3DRTYPE_TEXTURE) {
    ...
    for (UINT dst_level = 0; dst_level < levels_to_copy; ++dst_level) {
      ...
      // Keep the destination mirror in lockstep (DYNAMIC dst, see above). Same
      // sub-rect: src and dst share format and per-level dimensions here, so the
      // pitches and the row/column offsets match.
      if (dst_mirror) {
        uint8_t *dst_ptr = dst_mirror + dst->mirrorOffset(dst_level) + row_off + col_off;
  ...
  if (src_type == D3DRTYPE_VOLUMETEXTURE) {
    ...
    // Mirror the copy into a DYNAMIC dst's sysmem copy too (see the 2D branch);
    // null for pools without a mirror, which are served by readback.
    uint8_t *dst_mirror = dst->mirrorWriteBase();
    for (UINT dst_level = 0; dst_level < levels_to_copy; ++dst_level) {
      const UINT src_level = src_level_offset + dst_level;
      D3DVOLUME_DESC d{};
      if (FAILED(src->GetLevelDesc(src_level, &d)))
      ...
      if (dst_mirror) {
        uint8_t *dst_ptr = dst_mirror + dst->mirrorOffset(dst_level) + static_cast<size_t>(f) * slice_pitch +
                           static_cast<size_t>(t) * src_pitch + static_cast<size_t>(l) * bpp;
        const uint32_t copy_row_bytes = size.width * bpp;
        for (uint32_t z = 0; z < size.depth; ++z)
          for (uint32_t y = 0; y < size.height; ++y) {
            size_t off = static_cast<size_t>(z) * slice_pitch + static_cast<size_t>(y) * src_pitch;
            std::memcpy(dst_ptr + off, src_ptr + off, copy_row_bytes);
          }
      }
    }
    src->clearDirty();
...
MTLD3D9Device::GetRenderTargetData(IDirect3DSurface9 *pRenderTarget, IDirect3DSurface9 *pDestSurface) {
  ...
  // TODO: a SYSTEMMEM / SCRATCH texture sub-resource (CreateTexture +
  // GetSurfaceLevel) defers its host mirror until the first LockRect, so
  // cpuPtr() is still null here and the readback rejects. The standalone
  // offscreen-plain readback target the screenshot path uses has an eager
  // mirror and is unaffected; materialize the lazy mirror first when a
  // texture sub-resource needs to be a readback destination.
  if (!dst_base)
    return D3DERR_INVALIDCALL;
...
MTLD3D9Device::StretchRect(
  ...
  if (src_is_ds) {
    ...
    if (sd.Width != dd.Width || sd.Height != dd.Height)
      return D3DERR_INVALIDCALL;
    // A multisampled depth source would need a true depth resolve: Metal
    // does that through a render pass whose depth attachment carries a
    // resolveTexture and a depth resolve filter (DXVK routes it through
    // vkResolveImage's depth mode). The color resolve path below instead
    // builds a draw-based average-resolve PSO keyed on the dst format,
    // which is not color renderable for a depth texture and trips Metal
    // pipeline validation. Reject until a depth-resolve encode path lands
    // rather than crash on the invalid pipeline.
    if (sd.MultiSampleType != D3DMULTISAMPLE_NONE) {
...
MTLD3D9Device::DrawPrimitiveUP(
  ...
  auto clear_up_stream0 = [this]() {
    ...
    // A UP draw unbinds stream 0 with offset and stride 0, so a following
    // GetStreamSource reports them cleared. wined3d's DrawPrimitiveUP path
    // sets the stream source to (NULL, 0, 0); the public SetStreamSource
    // null-preserve rule does not apply here.
    m_streamOffsets[0] = 0;
    m_streamStrides[0] = 0;
  };
...
MTLD3D9Device::DrawIndexedPrimitiveUP(
  ...
  auto clear_up_state = [this]() {
    if (m_vertexBuffers[0].ptr())
      QueueRefOp(PendingRefOp::VertexBuffer0, nullptr);
    m_vertexBuffers[0] = nullptr;
    // Match wined3d's DrawIndexedPrimitiveUP: stream 0 unbinds with offset
    // and stride 0 (see clear_up_stream0).
    m_streamOffsets[0] = 0;
    m_streamStrides[0] = 0;
    if (m_indexBuffer.ptr())
      QueueRefOp(PendingRefOp::IndexBuffer, nullptr);
