...
MTLD3D9CubeTexture::MTLD3D9CubeTexture(
  ...
  // View cache — cube views inherit the parent's TextureCube type so
  // sample-bind callers can supply an sRGB-aliased / swizzled view
  // without changing the texture-type axis.
  WMT::Texture parentTex = m_texture->current()->texture();
  m_metalFormat = parentTex.pixelFormat();
  m_viewCache.reset(parentTex, WMTTextureTypeCube, m_metalFormat);
...
MTLD3D9CubeTexture::QueryInterface(REFIID riid, void **ppvObject) {
  D9_TRACE("IDirect3DCubeTexture9::QueryInterface");
  if (!ppvObject)
    return E_POINTER;
  *ppvObject = nullptr;
...
MTLD3D9CubeTexture::GetDevice(IDirect3DDevice9 **ppDevice) {
  D9_TRACE("IDirect3DCubeTexture9::GetDevice");
  if (!ppDevice)
...
MTLD3D9CubeTexture::SetPrivateData(REFGUID refguid, const void *pData, DWORD SizeOfData, DWORD Flags) {
  D9_TRACE("IDirect3DCubeTexture9::SetPrivateData");
...
MTLD3D9CubeTexture::GetPrivateData(REFGUID refguid, void *pData, DWORD *pSizeOfData) {
  D9_TRACE("IDirect3DCubeTexture9::GetPrivateData");
  if (!pSizeOfData)
    return D3DERR_INVALIDCALL;
...
MTLD3D9CubeTexture::FreePrivateData(REFGUID refguid) {
  D9_TRACE("IDirect3DCubeTexture9::FreePrivateData");
...
MTLD3D9CubeTexture::SetPriority(DWORD PriorityNew) {
  D9_TRACE("IDirect3DCubeTexture9::SetPriority");
...
MTLD3D9CubeTexture::GetPriority() {
  D9_TRACE("IDirect3DCubeTexture9::GetPriority");
  return m_priority;
}

void STDMETHODCALLTYPE
MTLD3D9CubeTexture::PreLoad() {
  D9_TRACE("IDirect3DCubeTexture9::PreLoad");
}

D3DRESOURCETYPE STDMETHODCALLTYPE
MTLD3D9CubeTexture::GetType() {
  D9_TRACE("IDirect3DCubeTexture9::GetType");
  return D3DRTYPE_CUBETEXTURE;
}

DWORD STDMETHODCALLTYPE
MTLD3D9CubeTexture::SetLOD(DWORD LODNew) {
  D9_TRACE("IDirect3DCubeTexture9::SetLOD");
...
MTLD3D9CubeTexture::GetLOD() {
  D9_TRACE("IDirect3DCubeTexture9::GetLOD");
  return m_lod;
}

DWORD STDMETHODCALLTYPE
MTLD3D9CubeTexture::GetLevelCount() {
  D9_TRACE("IDirect3DCubeTexture9::GetLevelCount");
  return m_level_count;
}

HRESULT STDMETHODCALLTYPE
MTLD3D9CubeTexture::SetAutoGenFilterType(D3DTEXTUREFILTERTYPE FilterType) {
  D9_TRACE("IDirect3DCubeTexture9::SetAutoGenFilterType");
...
MTLD3D9CubeTexture::GetAutoGenFilterType() {
  D9_TRACE("IDirect3DCubeTexture9::GetAutoGenFilterType");
  return m_autoGenFilter;
}

void STDMETHODCALLTYPE
MTLD3D9CubeTexture::GenerateMipSubLevels() {
  D9_TRACE("IDirect3DCubeTexture9::GenerateMipSubLevels");
...
MTLD3D9CubeTexture::GetLevelDesc(UINT Level, D3DSURFACE_DESC *pDesc) {
  D9_TRACE("IDirect3DCubeTexture9::GetLevelDesc");
  if (!pDesc)
    return D3DERR_INVALIDCALL;
...
MTLD3D9CubeTexture::GetCubeMapSurface(D3DCUBEMAP_FACES FaceType, UINT Level, IDirect3DSurface9 **ppCubeMapSurface) {
  D9_TRACE("IDirect3DCubeTexture9::GetCubeMapSurface");
  if (!ppCubeMapSurface)
    return D3DERR_INVALIDCALL;
  *ppCubeMapSurface = nullptr;
  if (FaceType >= 6 || Level >= m_level_count)
    return D3DERR_INVALIDCALL;
...
MTLD3D9CubeTexture::LockRect(
  ...
  D9_TRACE("IDirect3DCubeTexture9::LockRect");
  if (FaceType >= 6 || Level >= m_level_count)
    return D3DERR_INVALIDCALL;
  if ((m_usage & D3DUSAGE_AUTOGENMIPMAP) && Level != 0)
    return D3DERR_INVALIDCALL;
...
MTLD3D9CubeTexture::UnlockRect(D3DCUBEMAP_FACES FaceType, UINT Level) {
  D9_TRACE("IDirect3DCubeTexture9::UnlockRect");
  if (FaceType >= 6 || Level >= m_level_count)
    return D3DERR_INVALIDCALL;
  if ((m_usage & D3DUSAGE_AUTOGENMIPMAP) && Level != 0)
    return D3DERR_INVALIDCALL;
  ...
  // wined3d texture.c:1221 (top-level non-READONLY only).
  bool record_dirty = (Level == 0) && !m_levels[idx]->lockedReadOnly();
...
MTLD3D9CubeTexture::AddDirtyRect(D3DCUBEMAP_FACES FaceType, const RECT *pDirtyRect) {
  D9_TRACE("IDirect3DCubeTexture9::AddDirtyRect");
