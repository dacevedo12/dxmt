...
// IDirect3DVolume9 — single mip-level slice of an IDirect3DVolumeTexture9.
// Mirrors MTLD3D9Surface's role for 2D textures: holds a weak reference
// back to the parent container, exposes LockBox / UnlockBox over a CPU
// mirror, and shares the parent's underlying Metal handle.
//
// Lifetime: the parent volume-texture eagerly creates one MTLD3D9Volume
// per mip level (m_levels in the volume texture) under priv-pin
// (Com<,false>). GetVolumeLevel hands out a public ref on the cached
// slot; LockBox / UnlockBox route through the cached cpu_ptr into the
// parent's mirror.
//
// Reference: wined3d dlls/d3d9/volume.c.
