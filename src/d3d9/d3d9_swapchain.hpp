...
private:
  ...
  // m_dxmtTexture / m_viewCache) across consecutive slots so app-held
  ...
  // wsi::isMinimized probe (F5 from .local/sweep1-present-audit.md).
  // Stashed from the ctor's hEffectiveWindow argument, which the
  // interface layer resolves from params.hDeviceWindow / hFocusWindow
  // before construction. Null on headless chains.
  ...
  // wsi::getCurrentDisplayMode(MonitorFromWindow). Drives the
  // PresentationInterval=TWO/THREE/FOUR dwell math — apps that ask for
  // 2× vsync on a 120Hz display want a 16.67 ms dwell (60 fps), not the
  // 33.33 ms (30 fps) the prior 60Hz-hardcoded value gave them. Falls
  // back to 60.0 on detection failure (headless smokes, monitor query
  // returning zero, etc).
