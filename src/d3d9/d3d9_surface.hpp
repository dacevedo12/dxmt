...
public:
  ...
  // alias the parent cube texture, so its view cache must know it's
  // sitting on a Cube parent — newTextureView's slice-count rule (6
  // for Cube, 1 for 2D) is enforced at runtime by the Metal driver,
  // and a default-shape sRGB key would otherwise issue Type2D /
  // slice_count=6 against a Cube and fail validation. No default —
  // every caller must spell out the parent's type.
  // dxmtTexture: optional Rc<dxmt::Texture> wrapping the underlying
  // MTLTexture for Phase 3.5 chunk-emitcc lambdas to capture (so
  // ctx.access can attach the surface as an RT / sample source from
  // the encode thread). Per-level / per-face surfaces receive their
  // parent texture's Rc<>; standalone surfaces (CreateRenderTarget,
  // CreateDepthStencilSurface, CreateOffscreenPlainSurface, swapchain
  // backbuffer) build a fresh dxmt::Texture wrapping `texture`. Null
  // for surfaces that haven't been migrated yet — Phase 3.5 callers
  ...
  MTLD3D9Surface(
      ...
      Rc<dxmt::Texture> dxmtTexture = nullptr
  );
  ~MTLD3D9Surface();
  ...
  // Resolve a per-RT view (sRGB-aliased / format-swap). Default key
  // returns the underlying m_texture without creating a Metal view —
  // the common (non-sRGB) RT path stays allocation-free.
  WMT::Texture
  ...
  // Phase 3.5: chunk-emitcc draw lambdas capture this Rc<> to attach
  // the surface as a render target via ctx.access. Returns the parent
  // texture's Rc<> for per-level/per-face surfaces, the surface's own
  // for standalone allocations. May be null for surfaces that
  // predate the conversion — callers must check.
  ...
  // backing; the view cache is reset against the new parent.
  // Intended only for DEFAULT-pool surfaces with no mirror buffer;
  // m_buffer / m_cpu_ptr / m_owned_backing are not touched (the
  // swapchain backbuffer has none).
  void
  resetBacking(
      const D3DSURFACE_DESC &desc, WMT::Reference<WMT::Texture> texture, Rc<dxmt::Texture> dxmtTexture,
      WMTTextureType parentTextureType
  ) {
    m_desc = desc;
  ...
  // identical desc / format / non-lockable / no mirror buffer. The
  // view caches are swapped together with m_texture so each cache stays
  // consistent with its surface's new texture (cache entries are derived
  // from m_base, which moved with the swap).
  void
  SwapBacking(MTLD3D9Surface *other) {
    std::swap(m_texture, other->m_texture);
    std::swap(m_dxmtTexture, other->m_dxmtTexture);
    std::swap(m_viewCache, other->m_viewCache);
  }
...
private:
  ...
  // The cached m_desc / m_texture / m_metalFormat / m_viewCache are
  ...
  // Phase 3.5 chunk-lambda capture handle. For per-level / per-face
  // surfaces this points at the parent texture's dxmt::Texture; for
  // standalone surfaces (RT, DS, OffscreenPlain, swapchain backbuffer)
  // it owns the standalone allocation. Null for surfaces that haven't
  // been migrated yet — m_texture remains the source of truth for the
  // sync calling-thread path while phases 3.5+ land.
  ...
  // Per-surface view cache. Used for the RT-side of D3DRS_SRGBWRITEENABLE
  // (the render-pass attachment needs an sRGB-format view of the
  // parent texture so the PSO's declared sRGB format matches) and
  // for any future per-RT format aliasing. Default-key viewFor()
  // returns m_texture so the common (non-sRGB) RT path stays
  // allocation-free.
  ...
  // "surface storage IS this buffer, skip upload" — the Workstream B
  ...
  // reach (project_wow64_abi_gotchas memory). Owned by this object;
  // dtor frees via wsi::aligned_free. Null when m_buffer is using a
  // Metal-owned allocation (DEFAULT-pool RTs, future Private paths).
