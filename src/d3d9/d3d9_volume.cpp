...
MTLD3D9Volume::QueryInterface(REFIID riid, void **ppvObject) {
  D9_TRACE("IDirect3DVolume9::QueryInterface");
  if (!ppvObject)
    return E_POINTER;
  *ppvObject = nullptr;
...
MTLD3D9Volume::GetDevice(IDirect3DDevice9 **ppDevice) {
  D9_TRACE("IDirect3DVolume9::GetDevice");
  if (!ppDevice)
...
MTLD3D9Volume::SetPrivateData(REFGUID refguid, const void *pData, DWORD SizeOfData, DWORD Flags) {
  D9_TRACE("IDirect3DVolume9::SetPrivateData");
...
MTLD3D9Volume::GetPrivateData(REFGUID refguid, void *pData, DWORD *pSizeOfData) {
  D9_TRACE("IDirect3DVolume9::GetPrivateData");
  if (!pSizeOfData)
    return D3DERR_INVALIDCALL;
...
MTLD3D9Volume::FreePrivateData(REFGUID refguid) {
  D9_TRACE("IDirect3DVolume9::FreePrivateData");
...
MTLD3D9Volume::GetContainer(REFIID riid, void **ppContainer) {
  D9_TRACE("IDirect3DVolume9::GetContainer");
  if (!ppContainer)
    return D3DERR_INVALIDCALL;
  *ppContainer = nullptr;
...
MTLD3D9Volume::GetDesc(D3DVOLUME_DESC *pDesc) {
  D9_TRACE("IDirect3DVolume9::GetDesc");
  if (!pDesc)
    return D3DERR_INVALIDCALL;
  *pDesc = m_desc;
  return D3D_OK;
...
MTLD3D9Volume::LockBox(D3DLOCKED_BOX *pLockedVolume, const D3DBOX *pBox, DWORD Flags) {
  D9_TRACE("IDirect3DVolume9::LockBox");
  ...
  // pushLevelToGpu uses at d3d9_volume_texture.cpp:305.
...
MTLD3D9Volume::UnlockBox() {
  D9_TRACE("IDirect3DVolume9::UnlockBox");
  if (!m_locked)
    return D3DERR_INVALIDCALL;
  m_locked = false;
  // Container drives the GPU push at the volume-texture level
  // (UnlockBox passes the level through; the container reads our
  // m_locked_* fields to scope the upload). Reset bookkeeping so a
  // subsequent wider Lock doesn't inherit a narrower box.
  // Note: actual upload is triggered when the parent's UnlockBox
  // method runs — we just record the bounds here.
  return D3D_OK;
}
