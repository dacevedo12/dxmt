...
//
// Hot path cost when DXMT_D9_HOTCOUNTERS is enabled: 2 relaxed
...
// Present path, logging a single line. Env-gated by DXMT_D9_HOTCOUNTERS
...
struct D3D9HotCounters {
  ...
  // invocations. The micro counters bracket the EA-logo "1 fps"
  // suspects: flushOpenWorkMicros covers the chunk-emit work done
  // pre-Present (drainPendingClear + flushDeferredBlitWork); encode
  // is the Presenter::encodeCommands window (nextDrawable +
  // present-blit encode, combined — if this dominates we drill in
  // next iteration); total is the outer Present window so anything
  // not in a named bucket is visible.
  ...
  //   drawEncode      — per-DrawPrimitive call (entry to encoded into chunk).
...
// discipline as D9_HOT_BUMP: zero cost when DXMT_D9_HOTCOUNTERS is
// unset.
...
// Gates on construction: if DXMT_D9_HOTCOUNTERS is off, the timer
