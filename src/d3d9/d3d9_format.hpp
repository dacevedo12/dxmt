...
// formats dxmt does not support on the requested usage path.
//
// References:
//   - MGL: ~/refs/MGL/MGL/src/MGLTextures.m formatForType (the GL→Metal
//     lowering — we transcribe the matching D3D9→GL→Metal triples).
//   - Apple-Silicon quirk: D3DFMT_D24S8 must alias onto
//     Depth32Float_Stencil8; Apple GPUs reject the D24Unorm variant
//     at descriptor-validation time.
//
// The 'usage' argument selects the right alias for ambiguous D3DFORMATs:
// e.g. D3DFMT_D32F_LOCKABLE is Depth32Float either way, but D24S8
// resolves to D32FS8 only as a depth-stencil; as a sampler texture it
// is reported back via a different shape (typeless depth read).
//
// For the first wave of resource creation methods we only handle the
// formats games actually use as render targets: X8R8G8B8, A8R8G8B8,
// R5G6B5, A2R10G10B10, A8, plus the FP16/FP32 variants. New entries
// land alongside the methods that need them — keep additions paired
// with the validating CheckDeviceFormat path so the table stays
// honest.
...
// to Depth32Float_Stencil8 (project_apple_ds_format_alias.md), but the
// app's bias is sized in D24 units, so we return the D24 scale anyway.
...
// sampler-view path). Returned channels feed D3D9ViewKey.swizzle on
// the per-stage sample-bind path.
