...
// Internal: walk a D3D9 shader bytecode blob from its version token
// to the D3DSIO_END (0x0000FFFF) terminator and return the inclusive
// length in DWORDs. Returns 0 on failure (no terminator found).
//
// This is NOT a full DXSO instruction decoder — it skips comment
// tokens (the only token type whose body can legitimately contain
// 0xFFFF patterns) and otherwise assumes the END token only appears
// as the terminator. Real apps don't emit `def c0, ..` constants
// whose IEEE-754 bit pattern is 0x0000FFFF (~9.18e-41), so the scan
// is safe in practice. walk_dxso_shader (in
// dxso_decoder.hpp) handles real instruction-shape validation; this
// helper just sizes the bytecode buffer the iterator runs over.
...
// Optional one-line dump fired at CreateShader time. Gated by env
// var DXMT_D9_SHADER_DUMP=1 — off by default; when on, every
// successful walk lands a Logger::warn line in d3d9.log so we can
// see what real-world shader shapes the iterator is digesting
// without rebuilding to enable extra logging.
//
// When DXMT_D9_SHADER_DUMP_DIR=/path is also set, each unique
// bytecode (keyed by FNV-1a of the DWORD stream) is written once to
// `<path>/<vs|ps>_<hex-hash>.bin` for offline disassembly. Lets us
// inspect what real apps emit without depending on log capture from
// the wine console.
...
public:
  ...
  // Compile a per-IA-layout VS variant from the cached bytecode. The
  // pipeline builder calls this at draw time once the (decl, vs)
  // pair resolves into a concrete IA layout — the resulting
  // MTLFunction does manual buffer fetch instead of [[stage_in]].
  //
  // Cached on the shader keyed by the layout fingerprint
  // (slot_mask + per-element {reg, slot, offset, format}). Real apps
  // emit a small number of unique layouts per VS but issue hundreds
  // of thousands of draws against them — without the cache every draw
  // burns a fresh DXSO→AIR compile (and hammers AGX's compiler XPC
  // service hard enough to surface XPC_ERROR_CONNECTION_INTERRUPTED
  // at PSO link).
  //
  // The compile is synchronous, so `layout.elements` only needs to
  // outlive this call.
...
public:
  ...
  // Compile a per-(alpha-test, sampler-kind-layout) PS variant. The
  // pipeline builder calls this at draw time; the resulting MTLFunction
  // has the discard_fragment baked in (when alpha test is active) and
  // its sampler bindings typed per the host-supplied layout. Slots
  // marked DXSO_PS_SAMPLER_KIND_UNKNOWN fall back to the shader's own
  // inference (dcl-bearing SM 1.4+/2.0+ keep their declared kind; the
  // SM 1.0..1.3 default-to-Texture2D arm in dxso_compile still applies
  // when both the dcl and the host layout are silent).
  //
  // Cached on the shader keyed by (alpha-test-func, alpha-test-ref,
  // sampler-layout-hash) tuple. Real apps typically settle on a small
  // set of combinations; the cache stays modest and amortises across
  // hundreds of thousands of draws.
  WMT::Function compileVariant(
      ...
      const ::DXSO_SHADER_PS_BUMP_ENV_DATA *bump_env = nullptr
  );
