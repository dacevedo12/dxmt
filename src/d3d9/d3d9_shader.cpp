...
namespace {
// Token field shapes — see Microsoft's D3D9 shader-bytecode reference
// (also wined3d shader_sm1.c). Only the ones we need for the
// scan-to-END walk:
//   - END token: exact match 0x0000FFFF
//   - Comment token: low 16 bits == 0xFFFE; bits 16..30 hold the
//     comment body length in DWORDs (excluding the comment header)
constexpr DWORD kEndToken = 0x0000FFFFu;
constexpr DWORD kCommentOpcode = 0x0000FFFEu;
constexpr DWORD kOpcodeMask = 0x0000FFFFu;
constexpr DWORD kCommentSizeMask = 0x7FFF0000u;
constexpr unsigned kCommentSizeShift = 16;
// Hard upper bound — even SM3.0 shaders cap at a few KB. Anything
// past 64K DWORDs (256 KB) is malformed bytecode; bail rather than
// scanning forever.
//
// Caveat: the helper takes only a const DWORD*, so a missing-END
// blob from a hostile/fuzzed app can OOB-read up to 256 KB before
// this cap fires. Acceptable trade for now — wined3d/DXVK solve it
// with a full DxsoDecodeContext walker, which is the long-term shape.
constexpr size_t kMaxScanDwords = 65536;
} // namespace
...
shader_bytecode_dword_count(const DWORD *byte_code) {
  if (!byte_code)
    return 0;
  // Header (version) DWORD is at index 0. Tokens start at index 1.
  size_t i = 1;
  while (i < kMaxScanDwords) {
    DWORD token = byte_code[i];
    if (token == kEndToken)
      return i + 1;
    if ((token & kOpcodeMask) == kCommentOpcode) {
      // Skip the comment header DWORD plus its body.
      size_t body = (token & kCommentSizeMask) >> kCommentSizeShift;
      i += 1 + body;
      continue;
    }
    ++i;
...
namespace {
// One-shot env-var read. DXMT_D9_SHADER_DUMP=1 enables a per-create
...
shader_dump_enabled() {
  static const bool v = []() {
    const char *e = std::getenv("DXMT_D9_SHADER_DUMP");
    return e && e[0] && e[0] != '0';
  }();
  return v;
}
...
shader_dump_dir() {
  static const std::string v = []() -> std::string {
    const char *e = std::getenv("DXMT_D9_SHADER_DUMP_DIR");
...
log_shader_dump(
  ...
  // line — set DXMT_D9_SHADER_DUMP_DIR alone if the summary is too
  // noisy.
...
// Compile DXSO bytecode to a Metal MTLFunction in one shot. The
// function retains its parent MTLLibrary internally, so the caller
// only needs to hold the function reference. Errors land in d3d9.log
// as Logger::err and the caller gets a null function — the pipeline
// builder treats that as a hard mismatch the same way it treats empty
// bytecode.
//
// 32-bit WoW64 trap: the SM50_COMPILED_BITCODE.Data pointer returned
// by DXSOGetCompiledBitcode lives in the unix-side LLVM allocator (a
// 64-bit address), so a 32-bit caller cannot dereference it. Instead
// of memcpy'ing the bytes through the Windows side, the native 64-bit
// pointer is fed straight into newLibraryFromNativeBuffer, which
// packages it into a dispatch_data on the unix side without crossing
// the 4 GB barrier. Same shape as the d3d11 path
// (d3d11_shader.cpp:74-82). project_wow64_abi_gotchas memory.
...
compile_dxso_to_function(
  ...
  try {
    if (DXSOCompile(handle, args, "shader_main", &bc) == 0 && bc) {
      ...
      if (native_ptr && size) {
        // Wine main thread has no outer NSAutoreleasePool
        // (project_winemetal_autorelease memory) — newLibrary
        // autoreleases internally so the pool here keeps Create-time
        // leaks bounded.
  ...
  // Bridge the AIRCONV metallib dump (counter-keyed
  // {kind}_{n}.metallib under DXMT_AIRCONV_DUMP) to the bytecode
  // dump (hash-keyed {vs|ps}_{maj}_{min}_{hex-hash}.bin under
  // DXMT_D9_SHADER_DUMP_DIR) and the PSO failure log
  // (vs=0x… ps=0x…). Without this line, picking out which dumped
  // metallib corresponds to a failing PSO requires guessing from
  // counter ordering.
  //
  // Fires only when one of the two dump dirs is on, so production
  // runs stay quiet.
  if (function && (std::getenv("DXMT_D9_SHADER_DUMP_DIR") || std::getenv("DXMT_AIRCONV_DUMP"))) {
    uint64_t hash = fnv1a_dwords(byte_code, dwordCount);
    char line[200];
    std::snprintf(
...
// Fingerprint a DXSO_SHADER_IA_INPUT_LAYOUT_DATA into a single uint64
// suitable as a hash-map key. FNV-1a over (slot_mask, num_elements,
// each element's {reg, slot, offset, format, step_function, step_rate}).
// step_function + step_rate must be in the key — dxso_compile reads
// them at codegen time to emit per-instance vs per-vertex fetch, and
// two draws with the same vertex layout shape but different
// instancing settings collide silently (the per-instance draw reuses
// the per-vertex-compiled MSL and reads the wrong vertices). A
// collision otherwise degrades to a recompile, not a correctness bug.
static uint64_t
...
MTLD3D9VertexShader::compileVariant(const DXSO_SHADER_IA_INPUT_LAYOUT_DATA &layout, float point_size_override) {
  ...
  // Triage: dump unique layouts under DXMT_D9_LAYOUT_DUMP=1 so we can
  ...
  if (const char *en = std::getenv("DXMT_D9_LAYOUT_DUMP"); en && en[0] == '1') {
...
MTLD3D9VertexShader::QueryInterface(REFIID riid, void **ppv) {
  D9_TRACE("IDirect3DVertexShader9::QueryInterface");
  if (!ppv)
    return E_POINTER;
  *ppv = nullptr;
...
MTLD3D9VertexShader::GetDevice(IDirect3DDevice9 **ppDevice) {
  D9_TRACE("IDirect3DVertexShader9::GetDevice");
  if (!ppDevice)
...
MTLD3D9VertexShader::GetFunction(void *pData, UINT *pSizeOfData) {
  D9_TRACE("IDirect3DVertexShader9::GetFunction");
  ...
  // wined3d_shader_get_byte_code in dlls/wined3d/shader.c:2608.
  if (!pSizeOfData)
    return D3DERR_INVALIDCALL;
  ...
  // (dlls/wined3d/shader.c:2627) and DXVK
  // (src/d3d9/d3d9_shader.h:187) both omit the write here. Real apps
...
MTLD3D9PixelShader::compileVariant(
    ...
    const ::DXSO_SHADER_PS_BUMP_ENV_DATA *bump_env
) {
  // FNV-1a over the (alpha-test, sampler-kinds, point-sprite, bump-env)
  // tuple. Alpha args use 8 bits each; kinds are also 8 bits each;
  ...
  // Build the arg chain. The PSO_PIXEL_SHADER arg goes first when an
  // alpha test is active; the PS_SAMPLER_LAYOUT arg always follows so
  // the SM 1.x default-to-Texture2D arm in dxso_compile.cpp can override
  // its fallback with the host's per-slot kinds. PS_POINT_SPRITE
  // appends after sampler-layout when active — it's a zero-data
  // marker arg, the compile-side just notes its presence and routes
  // TEXCOORD<N> reads through [[point_coord]] instead of stage_in.
  // PS_BUMP_ENV appends last when present — carries per-stage
  // D3DTSS_BUMPENV* constants for TexBem / TexBemL / Bem emission.
  // Local stack copies are safe because DXSOCompile reads the chain
  // synchronously; bump_env is by-pointer because its 224-byte payload
  // shouldn't be copied per variant — the device site owns it.
  ...
  bool emit_alpha = alpha_test_func != 8 /* D3DCMP_ALWAYS */;
  if (emit_alpha) {
...
MTLD3D9PixelShader::QueryInterface(REFIID riid, void **ppv) {
  D9_TRACE("IDirect3DPixelShader9::QueryInterface");
  if (!ppv)
    return E_POINTER;
  *ppv = nullptr;
...
MTLD3D9PixelShader::GetDevice(IDirect3DDevice9 **ppDevice) {
  D9_TRACE("IDirect3DPixelShader9::GetDevice");
  if (!ppDevice)
    return D3DERR_INVALIDCALL;
  *ppDevice = ::dxmt::ref(static_cast<IDirect3DDevice9 *>(m_device));
  return D3D_OK;
...
MTLD3D9PixelShader::GetFunction(void *pData, UINT *pSizeOfData) {
  D9_TRACE("IDirect3DPixelShader9::GetFunction");
  if (!pSizeOfData)
    return D3DERR_INVALIDCALL;
  UINT need = static_cast<UINT>(bytecodeByteLength());
  if (!pData) {
