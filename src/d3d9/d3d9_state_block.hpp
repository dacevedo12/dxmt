...
// IDirect3DStateBlock9 captures a snapshot of device state and
// restores it on Apply. The full contract — wined3d
// dlls/d3d9/stateblock.c d3d9_stateblock_init plus device.c
// stateblock_init_default_state — covers ~30 state categories
// (transforms, render states, sampler states, texture stage states,
// FVF/decl, shaders, stream sources, indices, viewport, scissor,
// clip planes, materials, lights).
//
// Capture/Apply round-trips every state-block category: bitwise-
// copyable POD (render/sampler/texture-stage states, transforms,
// clip planes, viewport, scissor, FVF, stream offsets/strides,
// material, VS/PS constants F/I/B), reference-pinned slots (RTs,
// DS, textures, vertex/index buffers, vertex declaration, vertex
// shader, pixel shader), and the FFP lights vector. Reference slots
// use Com<,false>::operator= so the block holds a private ref to
// every captured target across its lifetime — an app that drops its
// last public ref to a bound resource between Capture and Apply
// still gets the resource back.
//
// Reference: wined3d dlls/d3d9/stateblock.c. DXVK
// src/d3d9/d3d9_stateblock.{cpp,h} uses a per-state-bit mask which
// is the right shape for the eventual real implementation; no Metal
// concept here, the whole class is D3D9 runtime bookkeeping.
...
private:
  ...
  // bindings (stateblock.c:903-1165) and Apply doesn't restore them.
