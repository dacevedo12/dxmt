...
public:
  ...
  // generateMipmaps — see ~/.claude/plans/d3d9-async-chunk-pivot.md
  // phase 5). They still create raw cmdbufs while the chunk pivot is
  // in progress; each call site goes through chunks once migrated.
  // d3d11 does not touch this field directly (uses CurrentChunk +
  // CommitCurrentChunk exclusively). Don't add new direct uses;
  // route through chunks.
