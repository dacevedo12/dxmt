...
enum WMTRenderCommandType : uint16_t {
  ...
  // dxso_compile.cpp:200+ emits per used sampler. Appended to keep the
...
/*
 * dxmt cross-boundary call counters (snapshot + reset). Bumped by the
 * WINE_UNIX_CALL macro in wineunixlib.h on every wine_unix_call dispatch
 * from the win32 side of winemetal. The aggregate variant returns the
 * sum; the breakdown variant fills a 256-slot array (indexed by the
 * dispatch code in __wine_unix_call_funcs[]) so callers can identify
 * which specific dispatch sites dominate the cross-WoW64 traffic.
 * Reading from a per-second tick and dividing by elapsed seconds gives
 * the actual rate, which d3d9_trace.cpp pairs with its high-level
 * counters to attribute cost.
 */
WINEMETAL_API unsigned long long winemetal_consume_unix_call_count(void);
WINEMETAL_API unsigned long long winemetal_consume_unix_call_breakdown(unsigned long long *counts_out);
