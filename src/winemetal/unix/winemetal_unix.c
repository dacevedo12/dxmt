...
const void *__wine_unix_call_funcs[] = {
    ...
    // Upstream apitrace block — indices 136..141 (UNIX_CALL hard-codes
    ...
    // DXSO block: indices 143..147, must match unix_dxso_* in
    // airconv_thunks.h. It sits immediately after the upstream apitrace
    // block (136..142); if upstream inserts more entries above this line,
    // move both this block and the airconv_thunks.h anchor forward in
    // lock-step.
...
const void *__wine_unix_call_wow64_funcs[] = {
    ...
    &_MTLCommandBuffer_property,
    &_MTLDevice_newTileRenderPipelineState,
    // Upstream apitrace block — indices 136..141, lock-step with the
    // 64-bit table above.
    &_WMTApitraceSessionEnsureOpen,
    &_WMTApitraceSessionClose,
    &_WMTApitraceSetCurrentD3DSequence,
    ...
    &_WMTApitracePresentDrawable,
    &_WMTApitraceSessionSealCheckpoint,
    // DXSO block — indices 143..147, lock-step with __wine_unix_call_funcs
    // above and with unix_dxso_* in airconv_thunks.h. Sits immediately
    // after the apitrace block (136..142).
