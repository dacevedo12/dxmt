...
test_colorfill(void) {
  ...
  if (lr.pBits) {
    ...
    // ColorFill writes an exact constant — no rasterizer/sampler
    // rounding — so the readback must match the requested colour
    // bit-for-bit at both the origin and the far corner.
    check_rgba_eq(origin, want);
    check_rgba_eq(far_p, want);
    sys->UnlockRect();
  }
  ...
  if (lr.pBits) {
    ...
    // Second fill must fully replace the first.
    check_rgba_eq(pixel, want);
    sys->UnlockRect();
  }
