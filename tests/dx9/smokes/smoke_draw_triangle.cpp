...
test_draw_triangle(void) {
  ...
  // A failure to create any of these would skip the draw and let the
  // test pass vacuously — assert the setup so a regression there fails.
  check_true(vs && ps && decl && vb && sys);
  if (vs && ps && decl && vb) {
    ...
    if (sys) {
      ...
      if (lr.pBits) {
        ...
        // The covered centre pixel must change; the uncovered corner must
        // still read back the clear colour. Booleans, not exact RGB —
        // rasterizer pixel-centre rounding makes the precise value vary.
