...
test_draw_trianglefan(void) {
  ...
  // Guard the readback: centre_is_clear() returns false on a null lock,
  // which would make the !is_clear check below pass vacuously.
  check_true(lr.pBits != nullptr);
  check_true(!centre_is_clear(lr)); // the fan covers the centre pixel
  sys->UnlockRect();
