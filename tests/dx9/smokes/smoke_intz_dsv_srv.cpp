...
test_intz_dsv_srv(void) {
  ...
  if (lr.pBits) {
    ...
    // The INTZ depth texel routed into the red lane must be non-zero (the
    // sample succeeded) and the textured fragment must have overridden the
    // clear colour.
    check_true(red_nonzero);
    check_true(not_clear);
    sys->UnlockRect();
  }
