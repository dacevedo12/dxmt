...
// Host-native unit test (no wine, no Metal, no device): the per-unit
// sampler-state power-on defaults. Exercises dxmt's
// init_default_sampler_states() directly — the table seeded at device
// creation / Reset, observable by any app that calls GetSamplerState
// before SetSamplerState.
//
// D3D9 has no dedicated Wine conformance test for sampler-state defaults,
// so the reference here is wined3d itself (dlls/wined3d/stateblock.c
// init_default_sampler_states) — the higher-priority reference, and the
// one dxmt's own implementation cites. Same values are the documented
// D3D9 defaults (WRAP addressing, POINT/POINT/NONE filtering, anisotropy 1).
