...
// Host-native unit test (no wine, no Metal, no device): D3DFVF ->
// D3DVERTEXELEMENT9 lowering. Exercises dxmt's build_fvf_decl_elements()
// directly — the same code path SetFVF uses to synthesise a declaration.
//
// The golden vectors are lifted verbatim from Wine's d3d9 conformance
// suite (dlls/d3d9/tests/device.c, fvf_to_decl_tests[] /
// test_fvf_decl_conversion). Wine's suite is the de-facto D3D9
// specification — Microsoft published no deterministic conformance
// probes — and is the same oracle wined3d and DXVK validate against. A
// pass here means dxmt's FVF lowering matches that reference field for
// field, provable on any machine in milliseconds without a GPU.
...
// Verbatim from Wine dlls/d3d9/tests/device.c:581 fvf_to_decl_tests[].
