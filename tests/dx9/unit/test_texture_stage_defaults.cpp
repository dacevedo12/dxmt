...
// Host-native unit test (no wine, no Metal, no device): the per-stage
// texture-stage-state power-on defaults. Exercises dxmt's
// init_default_texture_stage_states() directly — the table seeded at
// device creation / Reset, observable by any app that calls
// GetTextureStageState before SetTextureStageState.
//
// Expected values are transcribed from Wine's d3d9 conformance suite
// (dlls/d3d9/tests/device.c, test_texture_stage_states) — the de-facto
// D3D9 specification. Two stage-index-dependent rules: stage 0 gets the
// active MODULATE/SELECTARG1 ops while later stages DISABLE, and
// TEXCOORDINDEX defaults to the stage index.
