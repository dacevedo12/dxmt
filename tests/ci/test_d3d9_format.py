...
# The LLVM the tree is built against. Pinned because clang-format's output moves
# between releases: 21 and 22 format this code differently, so an unpinned gate
# fails on whichever machine did not format it rather than on the change.
EXPECTED_MAJOR = "21"
...
def main():
    ...
    major = next((part.split(".")[0] for part in version.split() if part[:1].isdigit()), None)
    if major != EXPECTED_MAJOR:
        print(f"FAIL: this tree is formatted with clang-format {EXPECTED_MAJOR}.x, found: {version}")
        print(f"      Releases disagree about the same file, so a different one reports")
        print(f"      formatting errors that are not there. The build toolchain carries")
        print(f"      {EXPECTED_MAJOR}.x; otherwise `pip install clang-format=={EXPECTED_MAJOR}.1.*`.")
        return 1
